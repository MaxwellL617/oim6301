...
def _():
    ...
    tax = 3.25
    total = cost + tax
    print(f'The cost is ${cost}. and the total is ${(cost + tax):.2f}')
    return (cost,)
...
@app.cell
def _():
    freight_charges = [15,32.50,23.75,70.20]
    items = ['textbook','laptop', 'monitor', 'keyboard']
    tax_rate = .12
    total_cost = 0
    phase = 0
    ...
    for charge in freight_charges:
        phase +=1 
        print(f'Charge Number {phase}:')
        added_tax = charge * tax_rate
        total_charge = added_tax + charge
        total_cost += total_charge
        print(f'Your {items[phase-1]} costs {total_charge:2f} after tax')

    print('You have spent: ')
    print(f'{total_cost} in total')
...
def _():
    ...
    # 1. value: 25
    # 2. name and assignment: square = value^2
    # 3. type: int
    # 4. list:
    # 5. index:
    # 6. loop:
